-- AI Survival Index alpha formalization schema.
-- Access model: the frontend talks to apps/api; apps/api talks to Supabase with
-- server-side credentials. RLS is enabled now, with no direct browser policies.
...
create table if not exists public.quiz_sessions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint quiz_sessions_completed_result_check check (
    status <> 'completed'
    or (
      primary_type is not null
      and secondary_type is not null
      and evolution_type is not null
...
create index if not exists quiz_sessions_profile_id_idx
  on public.quiz_sessions(profile_id);

create index if not exists quiz_sessions_primary_type_idx
  on public.quiz_sessions(primary_type);

create index if not exists quiz_answers_session_id_idx
  on public.quiz_answers(session_id);

create index if not exists archetype_results_profile_id_idx
  on public.archetype_results(profile_id);

create index if not exists archetype_results_primary_type_idx
  on public.archetype_results(primary_type);

create index if not exists friend_links_owner_profile_id_idx
  on public.friend_links(owner_profile_id);

create index if not exists friend_links_friend_profile_id_idx
  on public.friend_links(friend_profile_id);

create index if not exists share_events_profile_id_idx
  on public.share_events(profile_id);

create index if not exists share_events_session_id_idx
  on public.share_events(session_id);
...
grant all on table public.archetype_results to service_role;
grant all on table public.friend_links to service_role;
grant all on table public.share_events to service_role;
grant all on table public.memberships to service_role;

comment on table public.profiles is 'LINE user profile mirror. The line_user_id is stored server-side only.';
comment on table public.quiz_sessions is 'One quiz run. Completed sessions store the scored archetype keys.';
comment on table public.quiz_answers is 'Normalized answer events for each quiz scenario.';
comment on table public.archetype_results is 'Shareable scored result snapshot for one completed session.';
comment on table public.friend_links is 'LINE friend graph relation imported or inferred through share flows.';
comment on table public.share_events is 'Share and invite analytics events from LIFF or API flows.';
comment on table public.memberships is 'Future payment subscription state. No payment provider is connected yet.';
